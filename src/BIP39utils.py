...
# "input": "panel inhale anger power wreck cook million manage crawl grief inside cat",
wordList = '''\
...
def mnemonic_to_indices(mnemonic, to_hex=False):
    index_list = []
    for word in mnemonic.split():
        index = wordList.index(word)
        if to_hex:
            index_list.append(hex(index)[2:].zfill(4))
        else:
            index_list.append(str(index).zfill(4))
    return ' '.join(index_list)

def mnemonic_to_entropyHash(mnemonic):
    indexes = []
    entropy_hex = ""
    ...
    for word in mnemonic.split():
        ...
        mnemoninc_int = (mnemoninc_int << 11) + index
    if(mnemonic_len == 12):
        entropy_int = mnemoninc_int >> 4
        entropy_hex = hex(entropy_int)[2:].zfill(32)
    elif (mnemonic_len == 24):
        entropy_int = mnemoninc_int >> 8
        entropy_hex = hex(entropy_int)[2:].zfill(32)
    return bytes.fromhex(entropy_hex)

def indices_to_mnemonic(indices_list, from_hex=False):
    mnemonic = ""
    if (isinstance(indices_list,str)):
...
def get_seed(mnemonic_bytes, passphrase=""):
...
hashed_bip39_width_hex = 5
def generate_hashed_bip39(passphrase):
    wordList_hashed = []
    for word in wordList:
        word_bytes = word.encode('utf-8')
        word_hashed = get_seed(word_bytes, passphrase).hex()[-hashed_bip39_width_hex:]
        wordList_hashed.append(word_hashed)
    return wordList_hashed
def generate_hashed_mnemonic(mnemonic, passphrase):
    mnemonic_hashed = []
    for word in mnemonic.split():
        word_bytes = word.encode('utf-8')
        word_hashed = get_seed(word_bytes, passphrase).hex()[-hashed_bip39_width_hex:]
        mnemonic_hashed.append(word_hashed)
    return mnemonic_hashed
def validate_mnemonic(mnemonic):
    entropyHash = mnemonic_to_entropyHash(mnemonic)
    mnemonic_valid_checksum = get_mnemonic(entropyHash, len(mnemonic.split()))
    valid_mnemonic = True if (mnemonic == mnemonic_valid_checksum) else False
    return valid_mnemonic
def decode_hashed_mnemonic(hashed_mnemonic, wordList_hashed):
    mnemonic_len = len(hashed_mnemonic)
    indicies_list = []
    for hashed_word in hashed_mnemonic:
        indexes = [index for index, value in enumerate(wordList_hashed) if value == hashed_word]
        indicies_list.append(indexes)

    mnemonic_options = []

    for indicies in indicies_list:
        if len(indicies) == 1:
            mnemonic_options.append(indicies[0])
        else:
            mnemonic_options.append(indicies)

    mnemonic_permutations = expand_and_permute(mnemonic_options)
    valid_mnemonics = []
    for mnemonic_index_list in mnemonic_permutations:
        mnemonic = indices_to_mnemonic(mnemonic_index_list)
        if validate_mnemonic(mnemonic):
            valid_mnemonics.append(mnemonic)
        if(len(valid_mnemonics) == 1):
            returned_mnemonics = valid_mnemonics[0]
        else:
            returned_mnemonics = valid_mnemonics

    return returned_mnemonics

def has_duplicates(lst):
    return len(lst) != len(set(lst))
def print_duplicates(lst):
    # Creating a dictionary with the counts of each item in the list.
    count = Counter(lst)

    # Finding items where count is greater than 1 (duplicates) and printing them.
    duplicates = {item: count for item, count in count.items() if count > 1}
    for duplicate, occurrence in duplicates.items():
        print(f"{duplicate} is duplicated {occurrence} times.")
def expand_and_permute(lst):
    lists = [([x] if not isinstance(x, list) else x) for x in lst]
    return [list(item) for item in itertools.product(*lists)]

def datalog_hash_transactions(hashed_mnemonic_list):
    sum = 0
    transaction_str_list = []
    for word_hash in hashed_mnemonic_list:
        word_int = int(word_hash, 16)
        transaction_str = '0.' + str(word_int).zfill(8)
        transaction_str_list.append(transaction_str)
        sum = sum + float(transaction_str)
        print(f'        {transaction_str}')
    print(f'  SUM = {round(sum,8)}')
def log_hash_transactions_to_file(hashed_mnemonic_list):
    sum = 0
    transaction_str_list = []
    for word_hash in hashed_mnemonic_list:
        word_int = int(word_hash, 16)
    ...
    with open('transactionOutputList.txt', 'w') as filehandle:
        # Iterate through the list and write each item to the file
        for transaction_str in transaction_str_list:
            filehandle.write('%s\n' % transaction_str)

def read_hash_transactions():
    with open('transactionInputList.txt', 'r') as filehandle:
        hashTransactions = filehandle.readlines()
        hashTransactions = [transaction.strip() for transaction in hashTransactions]
        return hashTransactions

def hash_transactions_to_hashed_mnemonic(hash_transactions):
    hashed_mnemonic = []
    for transaction in hash_transactions:
        int_transaction = int(transaction[2:])
        hex_transaction = "{:0{width}x}".format(int_transaction, width=hashed_bip39_width_hex)
        hashed_mnemonic.append(hex_transaction)
    return hashed_mnemonic
def encrypt_word_to_int(word_str, passphrase):
    wordBytes = wordList[wordList.index(word_str)].encode('utf-8')
    encrypted_hex = get_seed(mnemonic_bytes=wordBytes, passphrase=passphrase)
    return encrypted_hex.hex()[-3:]

def encrypted_int_to_word(encoded_int, passphrase):
    decoded_word = None
    for word in wordList:
        wordList_word_bytes = word.encode('utf-8')
        print(get_seed(mnemonic_bytes=wordList_word_bytes, passphrase=passphrase).hex()[-3:])
        encoded_wordList_int = int(get_seed(mnemonic_bytes=wordList_word_bytes, passphrase=passphrase).hex()[-3:],16)
        if (encoded_int == encoded_wordList_int):
            decoded_word = word

    if (decoded_word == None):
        "ERROR: INVALID ENCODED INT"
